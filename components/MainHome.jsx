...
const MainHome = () => {
  ...
  const [isHomeFocused, setIsHomeFocused] = useState(false); // 새로운 상태 추가
  ...
  useEffect(() => {
    const backAction = () => {
      if (isSearchFocused) {
        ...
        setIsHomeFocused(false); // HomeFocus가 나타난 상태에서 뒤로가기 시 숨기기
        return true;
      }
      return false;
    };
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener("focus", () => {
      setIsSearchFocused(false);
      setIsHomeFocused(false); // Home 탭이 포커스될 때 상태 초기화
    });
